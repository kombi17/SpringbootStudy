...
public class SecurityConfiguration {
  ...
  public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
    http
        ...
        .rememberMe().key("key").rememberMeParameter("remember-me") //
        .tokenValiditySeconds(3600) // 쿠키 만료 시간 (1시간) default : 14일
        .alwaysRemember(false) // remember-me 기능 항상 실행 X
        .userDetailsService(userDetailService).and().formLogin((form) -> form // 로그인 설정
        ...
        .logout((logout) -> logout.permitAll() // 로그아웃 설정
            ...
            .logoutRequestMatcher(new AntPathRequestMatcher("/logout")).logoutSuccessUrl("/") // 로그아웃
                                                                                              // 시
                                                                                              // 이동할
                                                                                              // 페이지
